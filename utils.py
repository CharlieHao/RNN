...
def init_params(Mi, Mo):
    return np.random.randn(Mi, Mo) / np.sqrt(Mi + Mo)

def parity_data(nbit):
...
def parity_data_labeled(nbit):
    X, Y = parity_data(nbit)
    N, t = X.shape
...
def get_robert_frost():
    ...
    current_idx = 2
    sentences = []
    for line in open('/Users/zehaodong/research/RNN/data/robert_frost.txt'):
        line = line.strip()
        if line:
...
def get_wikipedia_data(n_files, n_vocab, by_paragraph=False):
    prefix = '/Users/zehaodong/research/RNN/data'
